...
class CircularTimerView : View {
    ...
    private var progressBackgroundColor = Color.TRANSPARENT
    ...
    private fun initPaints(context: Context, attrs: AttributeSet?) {
        ...
        backgroundColor = ta.getColor(R.styleable.CircularTimerView_backgroundColor, Color.TRANSPARENT)
        progressBackgroundColor = ta.getColor(R.styleable.CircularTimerView_progressBackgroundColor, Color.TRANSPARENT)
        strokeWidthDimension = ta.getFloat(R.styleable.CircularTimerView_strokeWidthDimension, 4f)
        backgroundWidth = ta.getFloat(R.styleable.CircularTimerView_backgroundWidth, 4f)
        roundedCorners = ta.getBoolean(R.styleable.CircularTimerView_roundedCorners, true)
        maxValue = ta.getFloat(R.styleable.CircularTimerView_maxValue, 100f)
        progressTextColor =+
            ta.getColor(R.styleable.CircularTimerView_progressTextColor, Color.WHITE)
        ...
        val bc = String.format("#%06X", 0x00FFFFFF and progressBackgroundColor)
        progressBarBackgroundPaint!!.color = Color.parseColor(bc)
        backgroundPaint = Paint(Paint.ANTI_ALIAS_FLAG)
        backgroundPaint!!.style = Paint.Style.FILL
        backgroundPaint!!.color = backgroundColor
        val bcfill = String.format("#%06X", 0x00FFFFFF and backgroundColor)
        backgroundPaint!!.color = Color.parseColor(bcfill)
        ta.recycle()
        textPaint = TextPaint()
        (textPaint as TextPaint).color = progressTextColor
        val c = String.format("#%06X", 0xFFFFFF and progressTextColor)
        (textPaint as TextPaint).color = Color.parseColor(c)
    ...
    fun setText(progressText: String?) {
        text = progressText?.let {
            // Split the input text into hours, minutes, and seconds
            val parts = it.split(":")
            if (parts.size == 3) {
                val hours = parts[0]
                val minutes = parts[1]
                ...
                val formattedSeconds = seconds.toIntOrNull()?.toString()?.padStart(2, '0')

                // Reconstruct the time string
                if (formattedSeconds != null) {
                    "$hours:$minutes:$formattedSeconds"
                } else {
                    it // Return as is if seconds cannot be parsed
                }
            } else {
                it // Return as is if not in the expected format
            }
        }
        invalidate()
    }
